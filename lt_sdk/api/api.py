...
def import_graph(graph_path, config, graph_type=None):
    ...
    if not graph_type:
        _, importer = graph_import.infer_import_graph_type(graph_path, config)
    else:
        graph_importer_cls = graph_import.get_graph_importer(graph_type)
        importer = graph_importer_cls(graph_path, config.sw_config)
...
def run_performance_simulation(graph, config):
    ...
    """
    perf_data = perfsim.simulate(graph, config.to_protobuf())

    return perf_data[0].execution_stats
...
def run_graph_pipeline(graph_path, config, export=False, export_path=""):
    ...
    Returns:
        lgf_graph.LightGraph: [description]
    """
...
def run_full_simulation(graph, data, config):
    ...
    Returns:
        ...
        Tuple containing the execution statistics for the performance simulation, and the
        outputs of the functional inference simulation.
