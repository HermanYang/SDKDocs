...
from lt_sdk.proto import lgf_pb2, node_filters
...
class RemovePadNodeTransform(base_transform.BaseTransform):
    ...
    @staticmethod
    def _get_same_output_height_and_width(input_height,
                                          input_width,
                                          stride_height,
                                          stride_width):
    ...
    @staticmethod
    def _get_same_padding(input_height,
                          input_width,
                          output_height,
                          output_width,
                          kernel_height,
        ...
        pad_along_height = max(
            (output_height - 1) * stride_height + kernel_height - input_height,
            0)
        pad_along_width = max(
            (output_width - 1) * stride_width + kernel_width - input_width,
            0)
        ...
        super().__init__(
            ...
            sw_config,
            {
                node_filters.which_oneof_filter(lgf_pb2.LNF.pad.DESCRIPTOR.name):
                    RemovePadNodeTransform(hw_specs,
                                           sw_config,
                                           sim_params)
            })
