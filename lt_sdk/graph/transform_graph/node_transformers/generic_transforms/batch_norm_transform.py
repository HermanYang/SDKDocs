...
class BatchNormTransform(electronic_op_transform.ElectronicOpTransform):
    ...
    def create_supported_nodes(self,
        ...
        if self.decompose():
            ...
            inputs = [input_edge, eff_bias_node.outputs[0]]
            inputs[lgf_pb2.FusedBatchNormNode.INPUT_INDEX] = input_edge

            add_output_edge = lgf_pb2.EdgeInfo()
            add_output_edge.CopyFrom(output_edge)
            add_output_edge.name = output_edge.name + "_add_eff_bias"

            add_node = self.create_simple_node(bn_name + "_add_eff_bias",
                                               lgf_pb2.LNF.vv_add.DESCRIPTOR.name,
                                               inputs,
                                               [add_output_edge],
                                               control_inputs)

            inputs = [add_node.outputs[0], eff_scale_node.outputs[0]]
            mul_node = self.create_simple_node(bn_name,
                                               lgf_pb2.LNF.vv_mul.DESCRIPTOR.name,
                                               inputs,
                                               [output_edge],
                                               control_inputs)

            return [mul_node, add_node, eff_bias_node, eff_scale_node]
        else:
