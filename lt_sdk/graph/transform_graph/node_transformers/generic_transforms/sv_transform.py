...
class SVTransform(electronic_op_transform.ElectronicOpTransform):
    ...
    def create_supported_nodes(self,
        ...
        """
        sv_node = self.create_simple_node(sv_name,
                                          self.sv_node_type(),
                                          [input_edge],
                                          [output_edge],
                                          control_inputs)
...
class ReluTransform(SVMaxTransform):
    ...
    def transform(self, relu_node, light_graph):
        ...
        return self.do_generic_transform(relu_node.name,
                                         relu_node.inputs[0],
                                         relu_node.outputs[0],
                                         relu_node.control_inputs,
                                         0)
...
class RsqrtTransform(SVPowTransform):
    ...
    def transform(self, rsqrt_node, light_graph):
        ...
        return self.do_generic_transform(rsqrt_node.name,
                                         rsqrt_node.inputs[0],
                                         rsqrt_node.outputs[0],
                                         rsqrt_node.control_inputs,
                                         -0.5)
