...
class TFSavedModelSplitTransform(tf_base_transform.TFSavedModelBaseTransform,
    ...
    def transform(self, split_node, light_graph):
        ...
        return self.do_generic_transform(split_node.name,
                                         split_node.inputs[input_indx],
                                         split_node.outputs,
                                         split_node.control_inputs,
                                         axis)
    ...
    def can_transform(self, node, light_graph):
        ...
        return len(node.inputs) in (2,
                                    3) and num == len(
                                        node.outputs) and super().can_transform(
                                            node,
                                            light_graph)
