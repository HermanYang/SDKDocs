...
class InstructionIssuer(object):
                ...
                if d in state.in_flight_pcs:
                    ...
                    for iss in self.in_flight:
                        if iss.decoded.instr.pc == d:
                            in_fl = iss
                    ok = dec.model.can_pipeline() and in_fl.decoded.model.can_pipeline()
                    perfsim_logging.log(
    ...
    def architectural_latency(self, dec):
        ret = self.perf_sim_params.mosaic.issue_clocks
    ...
    def next(self):
        ...
        issued = IssuedInstr(to_run, to_start + self.architectural_latency(to_run))
        perfsim_logging.log(
