...
from sdk2.graph.export_graph import graph_exporter, graph_exporter_map
from sdk2.graph.graph_collections import graph_collection
from sdk2.graph.run_graph.run_external_graph import external_graph_runner_map
from sdk2.graph.transform_graph import utils
from sdk2.proto import graph_types_pb2, inference_pb2, lgf_pb2
            ...
            for input_edge in n.inputs:
                if light_graph.has_node(input_edge.name):
                    try:
                        ...
                        logging.fatal(e)
                    if not utils.edges_match(
                            input_edge, matching_output_edge, check_consistent=True):
                        raise ValueError(
    ...
    def _create_output_map(output_edges, light_graph):
        ...
        for out_edge in output_edges:
            ...
            if matching_edge is not None:
                output_map[(out_edge.name, out_edge.port)] = (matching_edge.name,
                                                              matching_edge.port)
            else:
