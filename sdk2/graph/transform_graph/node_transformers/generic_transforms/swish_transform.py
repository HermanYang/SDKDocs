from sdk2.graph.transform_graph.node_transformers.generic_transforms import (
    sigmoid_transform, vv_transform)
...
class SwishTransform(sigmoid_transform.SigmoidTransform, vv_transform.VVMulTransform):
    ...
    def transform(self, swish_node, light_graph):
        ...
        sigmoid_nodes = sigmoid_transform.SigmoidTransform.create_supported_nodes(
            self, sigmoid_output_edge.name, swish_node.inputs[0], sigmoid_output_edge,
            swish_node.control_inputs)

        vv_mul_node = vv_transform.VVMulTransform.create_supported_nodes(
            self, swish_node.name, swish_node.inputs[0], sigmoid_nodes[0].outputs[0],
            swish_node.outputs[0], swish_node.control_inputs)[0]
