from sdk2.graph.transform_graph.node_transformers.generic_transforms import (
    electronic_op_transform, exp_transform, sv_transform, vv_transform)
...
class TanhTransform(electronic_op_transform.ElectronicOpTransform):
    ...
    def create_supported_nodes(self, tanh_name, input_edge, output_edge, control_inputs):
        ...
        sv_mul_node = sv_transform.SVMulTransform(*common_args).create_supported_nodes(
            sv_mul_output_edge.name, input_edge, sv_mul_output_edge, control_inputs,
            2)[0]
        ...
        exp_node = exp_transform.ExpTransform.create_supported_nodes(
            self, exp_output_edge.name, sv_mul_node.outputs[0], exp_output_edge,
            control_inputs)[0]
        ...
        numerator_node = sv_transform.SVAddTransform(
            *common_args).create_supported_nodes(numerator_output_edge.name,
                                                 exp_node.outputs[0],
                                                 numerator_output_edge, control_inputs,
                                                 -1)[0]
        denominator_node = sv_transform.SVAddTransform(
            *common_args).create_supported_nodes(denominator_output_edge.name,
                                                 exp_node.outputs[0],
                                                 denominator_output_edge, control_inputs,
                                                 1)[0]

        vv_div_node = vv_transform.VVDivTransform(*common_args).create_supported_nodes(
            tanh_name, numerator_node.outputs[0], denominator_node.outputs[0],
    ...
    def transform(self, tanh_node, light_graph):
        ...
        return self.do_generic_transform(tanh_node.name, tanh_node.inputs[0],
                                         tanh_node.outputs[0], tanh_node.control_inputs)
