...
class PoolTransform(mean_transform.MeanTransform):
    ...
    def _create_mean_nodes(self, pool_name, input_edge, output_edge, control_inputs,
                           image_attr):
        h_indx, w_indx, _ = self._get_height_and_width_indices(image_attr)
        return mean_transform.MeanTransform.create_supported_nodes(
            self, pool_name, input_edge, output_edge, control_inputs, [h_indx, w_indx],
            True)

    def create_supported_nodes(self, pool_name, input_edge, output_edge, control_inputs,
                               pooling_type, kernel_size, strides, padding, data_format):
        """
        ...
        if self._is_mean(input_edge, pooling_type, image_attr):
            self._create_mean_nodes(pool_name, input_edge, output_edge, control_inputs,
                                    image_attr)

        pool_node = self.create_simple_node(pool_name, lgf_pb2.LNF.pool.DESCRIPTOR.name,
