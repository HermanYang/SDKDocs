from sdk2.graph.transform_graph.node_transformers.generic_transforms import (
    exp_transform, reduce_sum_transform, vv_transform)
...
class SoftmaxTransform(exp_transform.ExpTransform,
    ...
    def create_supported_nodes(self, softmax_name, input_edge, output_edge,
        ...
        reduce_sum_node = reduce_sum_transform.ReduceSumTransform.create_supported_nodes(
            self, reduce_sum_output_edge.name, exp_node.outputs[0],
            reduce_sum_output_edge, control_inputs, [axis], True)[0]

        vv_div_node = vv_transform.VVDivTransform.create_supported_nodes(
            self, softmax_name, exp_node.outputs[0], reduce_sum_node.outputs[0],
