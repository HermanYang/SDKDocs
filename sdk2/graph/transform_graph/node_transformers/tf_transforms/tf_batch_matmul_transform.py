...
from sdk2.graph.transform_graph.node_transformers.generic_transforms import (
    base_transform, identity_transform, matmul_transform, reshape_transform,
    stack_transform, unstack_transform)
...
class TFSavedModelBatchMatMulV2Transform(base_transform.BaseTransform,
    ...
    def _unstack_edges(self, edge, num):
        ...
        unstack_node = unstack_transform.UnstackTransform(
            *common_args).create_supported_nodes(unstacked_edges[0].name, reshape_edge,
                                                 unstacked_edges, [], 0)[0]
    ...
    def _stack_edges(self, edges, out_edge):
        ...
        stack_node = stack_transform.StackTransform(*common_args).create_supported_nodes(
            stack_edge.name, edges, stack_edge, [], 0)[0]

        if len(out_edge.shape.d) > 3:
            out_node = reshape_transform.ReshapeTransform(
                *common_args).create_supported_nodes(out_edge.name, stack_edge, out_edge,
                                                     [])[0]
        else:
    ...
    def create_supported_nodes(self, batch_matmul_name, input_edge, weight_edge,
                               output_edge, control_inputs, transpose_inputs,
                               transpose_weights, num):
        ...
        for i, (new_input, new_weight) in enumerate(
            ...
            matmul_node = matmul_obj.create_supported_nodes(matmul_edge.name, new_input,
                                                            new_weight, matmul_edge,
                                                            control_inputs,
                                                            transpose_inputs,
                                                            transpose_weights)
