...
class TFSavedModelSquaredDifferenceTransform(tf_base_transform.TFSavedModelBaseTransform,
    ...
    def create_supported_nodes(self, sq_diff_name, input0_edge, input1_edge, output_edge,
                               control_inputs):
        """
        ...
        vv_sub_node = vv_transform.VVSubTransform(*common_args).create_supported_nodes(
            vv_sub_output_edge.name, input0_edge, input1_edge, vv_sub_output_edge,
            control_inputs)[0]

        sv_pow_node = sv_transform.SVPowTransform(*common_args).create_supported_nodes(
            sq_diff_name, vv_sub_node.outputs[0], output_edge, control_inputs, 2)[0]
    ...
    def transform(self, sq_diff_node, light_graph):
        ...
        return self.do_generic_transform(sq_diff_node.name, sq_diff_node.inputs[0],
                                         sq_diff_node.inputs[1], sq_diff_node.outputs[0],
