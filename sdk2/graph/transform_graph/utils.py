...
from sdk2.proto import common_pb2, dtypes_pb2, inference_pb2, lgf_pb2

LT_UNSET = "_LT_UNSET_:0"
SCALE_EPS_VALUE = np.finfo(np.float64).eps

NP_TYPES = {
    np.dtype(np.float32): (dtypes_pb2.DT_FLOAT, 32),
    np.dtype(np.float64): (dtypes_pb2.DT_FLOAT, 32),
    np.dtype(np.int8): (dtypes_pb2.DT_INT, 8),
    np.dtype(np.int16): (dtypes_pb2.DT_INT, 16),
    np.dtype(np.int32): (dtypes_pb2.DT_INT, 32),
    np.dtype(np.int64): (dtypes_pb2.DT_INT, 64),
    np.dtype(np.uint8): (dtypes_pb2.DT_UINT, 8),
    np.dtype(np.uint16): (dtypes_pb2.DT_UINT, 16),
    np.dtype(np.bool): (dtypes_pb2.DT_BOOL, 1),
    np.dtype(np.complex64): (dtypes_pb2.DT_COMPLEX, 64),
    np.dtype(np.complex128): (dtypes_pb2.DT_COMPLEX, 128),
}
...
def dtype_pb_to_np_dtype(lgf_dtype):
    ...
    if lgf_dtype.t == dtypes_pb2.DT_FLOAT:
        ...
        raise ValueError("Cannot convert {} {} to numpy dtype".format(
            dtypes_pb2.Type.Name(lgf_dtype.t), lgf_dtype.p))
...
def array_to_tensor_pb(array, dtype_pb, batch_dim_indx=-1):
    ...
    tensor_pb.tensor_content = array.astype(dtype_pb_to_np_dtype(dtype_pb)).tostring()
