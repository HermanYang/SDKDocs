...
class CollapseSupportedSubgraphs(graph_transform.GraphTransform):
    ...
    def _get_unsupported_path_dict_for_node(self, node, node_cache,
            ...
            if not is_input_node:
                for e in node.inputs:
                    if e.name not in node_cache:
                        continue
                    unsupported_path_dicts[node.name][e.name] = not (
                        node_cache[node.name].supported and node_cache[e.name].supported)
                    path_dict = self._get_unsupported_path_dict_for_node(
                        node_cache[e.name], node_cache, unsupported_path_dicts)
                    for name, status in path_dict.items():
                        new_status = unsupported_path_dicts[node.name][e.name] or status
                        unsupported_path_dicts[
                            node.name][name] = unsupported_path_dicts[node.name].get(
                                name, False) or new_status

        return unsupported_path_dicts[node.name]

    def _unsupported_path_exists(self, src_node, dst_node, node_cache,
                                 unsupported_path_dicts):
        """
        ...
        return self._get_unsupported_path_dict_for_node(
            ...
            node_cache,
            unsupported_path_dicts,
        ).get(src_node.name, False)

    def _safe_to_add_node_to_subgraph(self, candidate_node, new_subgraph_nodes,
                                      node_cache, unsupported_path_dicts):
        """
        Params:
        ...
        for node in new_subgraph_nodes:
            if (self._unsupported_path_exists(candidate_node, node, node_cache,
                                              unsupported_path_dicts)
                    or self._unsupported_path_exists(node, candidate_node, node_cache,
                                                     unsupported_path_dicts)):
                return False
    ...
    def _get_supported_subgraph_lists(self, light_graph):
        ...
        for node in light_graph_nodes:
            if (supported_filter.matches(node, light_graph)
                ...
                for candidate_node in light_graph.bfs(node,
                    ...
                    if (candidate_node.name not in visited_node_names
                            and self._safe_to_add_node_to_subgraph(
                                candidate_node, new_subgraph_nodes, node_cache,
                                unsupported_path_dicts)):
                        new_subgraph_nodes.append(candidate_node)
                        visited_node_names.add(candidate_node.name)
                subgraphs.append(
                    self._subgraph_nodes_to_subgraph(new_subgraph_nodes, light_graph))
    ...
    def _is_cyclic(self, subgraphs, light_graph):
        ...
        output_edge_map = {(e.name, e.port): subgraph_name_format.format(i)
                           for i, (sg, _) in enumerate(subgraphs)
                           for e in sg.output_edges()}
        ...
        def check_cyclic_utils(node_name):
            ...
            input_edges = node.input_edges() if isinstance(
                node, lgf_graph.LightGraph) else node.inputs
                    ...
                    if not all_while_nodes:
                        ...
                        for n in dfs_node_stack[indx:]:
                            ...
                            if isinstance(subgraph, lgf_graph.LightGraph):
                                logging.error(
                                    "%s: nodes %s, input edges %s, output edges %s", n,
