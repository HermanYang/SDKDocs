...
from sdk2.graph.import_graph import tf_saved_model_base_importer, tf_saved_model_importer
...
from sdk2.graph.transform_graph.node_transformers.generic_transforms import \
    base_transform
from sdk2.proto import graph_types_pb2, lgf_pb2, ops_pb2, transform_result_pb2
...
class AddCastNodes(graph_transform.GraphTransform):
    ...
    def process_cast_node(cast_node, input_node, output_node, sw_config):
        if input_node.supported and output_node.supported:
            ...
            if graph_type in {
                    graph_types_pb2.TFSavedModel, graph_types_pb2.TFTrainingSavedModel,
                    graph_types_pb2.TFTrainingCheckpoint
            }:
                cast_node.original.CopyFrom(
                    AddCastNodes._get_original_tf_node(cast_node, sw_config))
            else:
                raise ValueError(
            ...
            for input_edge in n.inputs:
                if light_graph.has_node(input_edge.name):
                    ...
                    if (input_edge.dtype.SerializeToString() !=
                        ...
                        transforms.append(
                            self._add_cast_node(n, input_edge, input_node,
