...
from sdk2.graph.run_graph import histogram_graph_runner
from sdk2.graph.transform_graph.calibration import convert_to_adc_scale_calibration_graph
from sdk2.graph.transform_graph.graph_transformers import apply_node_map
from sdk2.proto import calibration_pb2, common_pb2, sim_params_pb2


def get_scales_data(hist_coll, light_graph, convert_to_calib_graph, hw_specs, sw_config,
                    sim_params):
    # Get opu nodes
    ...
    scales = hist_coll.get_quant_scales(sw_config.adc_scale_quantization_method,
                                        sw_config, precisions, common_pb2.QB_NONE)
    ...
    with graph_collection.GraphCollection() as graph_coll:
        ...
        convert_to_calib_graph = (
            convert_to_adc_scale_calibration_graph.ConvertToADCScaleCalibrationGraph(
                hw_specs, sw_config, sim_params, hist_coll))
        calib_graph = convert_to_calib_graph.process_transforms(light_graph)

        runner = histogram_graph_runner.HistogramGraphRunner(
            calib_graph, hw_specs, sw_config, get_virtual_sim_params(sim_params),
            graph_coll)
        runner.run(calibration_data)

        # Get scales data
        logging.info("-Computing Scales")
        adc_scales_data = get_scales_data(hist_coll, light_graph, convert_to_calib_graph,
