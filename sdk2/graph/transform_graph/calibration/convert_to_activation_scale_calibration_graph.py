...
from sdk2.graph.transform_graph.node_transformers.generic_transforms import \
    base_transform
from sdk2.proto import common_pb2, lgf_pb2, transform_result_pb2
...
class ConvertToActivationScaleCalibrationGraph(graph_transform.GraphTransform):
    ...
    def insert_collect_hist_node(edge, sw_config, hist_key, num_bins, hist_coll):
        ...
        if insert_cast:
            ...
            to_reroute.append(
                (edge_reroute, [], collect_hist_output_edge, reverse_cast_output_edge))
    ...
    def _insert_collect_hist_node(self, edge, node_info):
        ...
        return self.insert_collect_hist_node(
            edge, self._sw_config, self._get_new_hist_key(edge_tuple, node_info),
            self._sw_config.activation_scale_num_bins, self._hist_coll)
