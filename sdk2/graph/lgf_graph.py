from sdk2.proto import lgf_pb2, node_filters, ops_pb2
...
class LightGraph(object):
    ...
    def __init__(self,
        ...
        self._node_to_input_node_names = {
            k: sorted(v) for k, v in self._node_to_input_node_names.items()
        }
        self._node_to_output_node_names = {
            k: sorted(v) for k, v in self._node_to_output_node_names.items()
        }
            ...
            for parent_node in current_nodes:
                ...
                for child_node in child_nodes:
                    if (child_node.name not in visited_node_names
                            and node_filter.matches(child_node, self)):
