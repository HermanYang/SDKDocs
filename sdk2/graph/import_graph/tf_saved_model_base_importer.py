...
class ImportTFSavedModelBase(graph_importer.ImportGraph):
    ...
    DATA_TYPE_MAP = {
        tf.float32: (dtypes_pb2.DT_FLOAT, 32),
        tf.float64: (dtypes_pb2.DT_FLOAT, 64),
        tf.bfloat16: (dtypes_pb2.DT_BFLOAT, 16),
        tf.int8: (dtypes_pb2.DT_INT, 8),
        tf.int16: (dtypes_pb2.DT_INT, 16),
        tf.int32: (dtypes_pb2.DT_INT, 32),
        tf.int64: (dtypes_pb2.DT_INT, 64),
        tf.uint8: (dtypes_pb2.DT_UINT, 8),
        tf.uint16: (dtypes_pb2.DT_UINT, 16),
        tf.string: (dtypes_pb2.DT_STRING, 0),
        tf.bool: (dtypes_pb2.DT_BOOL, 1),
        tf.complex64: (dtypes_pb2.DT_COMPLEX, 64),
        tf.complex128: (dtypes_pb2.DT_COMPLEX, 128),
        tf.qint8: (dtypes_pb2.DT_QINT, 8),
        tf.qint16: (dtypes_pb2.DT_QINT, 16),
        tf.qint32: (dtypes_pb2.DT_QINT, 32),
        tf.quint8: (dtypes_pb2.DT_QUINT, 8),
        tf.quint16: (dtypes_pb2.DT_QUINT, 16),
    }

    REV_DATA_TYPE_MAP = {
        (dtypes_pb2.DT_FLOAT, 32): tf.float32,
        (dtypes_pb2.DT_FLOAT, 64): tf.float64,
        (dtypes_pb2.DT_BFLOAT, 16): tf.bfloat16,
        (dtypes_pb2.DT_INT, 8): tf.int8,
        (dtypes_pb2.DT_INT, 16): tf.int16,
        (dtypes_pb2.DT_INT, 32): tf.int32,
        (dtypes_pb2.DT_INT, 64): tf.int64,
        (dtypes_pb2.DT_UINT, 8): tf.uint8,
        (dtypes_pb2.DT_UINT, 16): tf.uint16,
        (dtypes_pb2.DT_STRING, 0): tf.string,
        (dtypes_pb2.DT_BOOL, 1): tf.bool,
        (dtypes_pb2.DT_COMPLEX, 64): tf.complex64,
        (dtypes_pb2.DT_COMPLEX, 128): tf.complex128,
        (dtypes_pb2.DT_QINT, 8): tf.qint8,
        (dtypes_pb2.DT_QINT, 16): tf.qint16,
        (dtypes_pb2.DT_QINT, 32): tf.qint32,
        (dtypes_pb2.DT_QUINT, 8): tf.quint8,
        (dtypes_pb2.DT_QUINT, 16): tf.quint16
    }
    ...
    def _get_feed_dict(self, sess, unknown_dim_size):
        ...
        for inp in self._input_tensor_names:
            ...
            shape = self.tf_tensorshape_to_lgf_tensorshape(
                sess.graph.get_tensor_by_name(inp).shape, update_batch_dim_indx=False)
            ...
            feed_dict[sess.graph.get_tensor_by_name(inp)] = utils.tensor_pb_to_array(
                named_tensor.data, utils.dtype_pb_to_np_dtype(named_tensor.data.dtype))
        ...
        for i, (s1, s2) in enumerate(zip(shape_1, shape_2)):
            if s1 != s2:
                ...
                if (dilation_factor == s2 / self.RUN_2_BATCH_SIZE
                    ...
                    logging.warning(
                        "Found non-matching dimension %d for tensor %s: %d, %d", i,
                        ten_name, s1, s2)
        ...
        if self._input_edges:
            ...
            for i in range(len(arrays_1)):
                ...
                self._tensor_shapes[ten_name] = self.np_shape_to_lgf_tensorshape(
                    array_1.shape, update_batch_dim_indx=False)
    ...
    def _tf_node_def_to_lnf(self, tf_node_def):
        ...
        output_tensor_names = [
            "{0}:{1}".format(lnf.name, port) for port in range(max_port)
        ]
        lnf.outputs.extend([
