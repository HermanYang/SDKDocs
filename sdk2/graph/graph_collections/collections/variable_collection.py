...
class VariableCollection(object):
    ...
    lib = ctypes.CDLL(
        os.path.join(os.path.dirname(__file__), "lib_variable_collection.so"))

    lib.GetVariable.argtypes = (ctypes.c_void_p, ctypes.POINTER(ctypes.c_char),
                                ctypes.POINTER(ctypes.c_uint))
    lib.GetVariable.restype = ctypes.POINTER(ctypes.c_char)

    lib.AddVariable.argtypes = (ctypes.c_void_p, ctypes.POINTER(ctypes.c_char),
    ...
    def get_variable(self, node_name):
        ...
        tensor_ptr = self.lib.GetVariable(self._obj, node_name.encode("utf-8"),
    ...
    def add_variable(self, node_name, tensor_pb):
        ...
        self.lib.AddVariable(self._obj, node_name.encode("utf-8"), tensor_data,
                             len(tensor_data))
