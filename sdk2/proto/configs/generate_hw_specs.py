...
from sdk2.proto import hw_spec_pb2, subgraph_binary_pb2
...
def generate_mosaic_bravo(num_umem_banks=3, num_wavelengths=4, use_ideal_mode=True):
    hw_specs = generate_mosaic(
        input_dim=64,
        input_precision=InputPrecision(precision=4, signed=False, time_multiplex=True),
        phase_precision=PhasePrecision(precision=6, signed=False, time_multiplex=False),
        output_precision=8,
...
SWEEPS = {
    SWEEP_MOSAIC:
        param_sweep.ParamSweep(SWEEP_MOSAIC, generate_mosaic, input_dim=[16, 32, 64])
}
...
def generate(out_dir=None, config_name=CONFIG_MOSAIC_BRAVO_IDEAL, sweep_name=None):
    ...
    if config_name:
        ...
        utils.write_hw_specs(func(**kwargs),
                             os.path.join(out_dir, "{0}.pb.txt".format(config_name)))

    paths = {}
    if sweep_name:
        for obj, cfg_name in SWEEPS[sweep_name].generate():
            config_path = os.path.join(out_dir,
                                       "{0}_{1}.pb.txt".format(sweep_name, cfg_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--out_dir",
                        default=os.path.join(os.path.expanduser("~"), "configs"),
                        type=str,
                        help="Name of set of hardware configs.")
