...
from sdk2.common import py_graph_test_util, py_test_util
...
from sdk2.verification import performance_sweep_map
...
class BNOffsetTransform(electronic_op_transform.ElectronicOpTransform):
    ...
    def transform(self, offset_node, light_graph):
        ...
        offset_node.const.value.CopyFrom(
            utils.array_to_tensor_pb(new_arr, offset_node.const.value.dtype))
...
def collect_data_dict(data_d, shard_outs):
    for batch in shard_outs.batches:
        for named_tensor in batch.results:
            data_d.setdefault(named_tensor.edge_info.name, []).append(
                utils.tensor_pb_to_array(named_tensor.data, np.float32))


def get_layer_errors(workload, output_dir, opu_only=False, max_nodes=32, fix_bns=True):
    num_threads = 64
    ...
    sw_config = generate_sw_config.generate_standard_sw_config(
        perf_sweep.graph_type(), num_threads_scales=num_threads)
    sw_config.cache_dir = output_dir
    sim_params = generate_sim_params.generate_mosaic(num_runtime_threads=num_threads)

    sw_config.sweep_info.py_batch_size = perf_sweep.py_batch_size()
    sw_config.sweep_info.num_inference_samples = 5
    ...
    full_graph_pipeline.main(orig_path, graph_types_pb2.LGFProtobuf, modified_path,
                             graph_types_pb2.LGFProtobuf, calibration_data, hw_specs,
    ...
    runner = graph_runner.GraphRunner(modified_pruned_lgf, hw_specs, sw_config,
                                      sim_params)
    ...
    for name in orig_data.keys():
        errors[name] = py_graph_test_util.GraphTestCase.relative_error(
            np.concatenate(orig_data[name], axis=1),
            np.concatenate(mod_data[name], axis=1))
    ...
    if fix_bns:
        ...
        for e in opu_edges:
            if e.name.endswith("FusedBatchNorm"):
                ...
                corrections[node.inputs[1].name] = np.mean(mod, axis=0) - np.mean(orig,
                                                                                  axis=0)

        bn_offset_filter = node_filters.name_in_filter("bn_offset")
        offset_transformer = BNOffsetTransform(corrections, hw_specs, sim_params,
                                               sw_config)
        ...
        lgf_graph.LightGraph.write_lgf_pb(new_graph.as_lgf_pb(),
                                          os.path.join(output_dir, "fixed_bns.pb"))
...
def main(workload_name, output_dir, **kwargs):
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    errors, names, errors_pb = get_layer_errors(workload_name, output_dir, **kwargs)
    with open(os.path.join(output_dir, "relative_error_data.pb"), "wb") as f:
        f.write(errors_pb.SerializeToString())

    _plot_errors(errors, names,
                 os.path.join(output_dir, "{0}_errors.png".format(workload_name)))
