...
from sdk2.common import context_manager_utils
...
class PyInferenceRunner(object):
    ...
    lib.Run.argtypes = (ctypes.c_void_p, ctypes.c_char_p, ctypes.c_uint, ctypes.c_char_p,
                        ctypes.c_uint, ctypes.c_char_p, ctypes.c_uint, ctypes.c_char_p,
                        ctypes.c_uint, ctypes.c_char_p, ctypes.c_uint, ctypes.c_void_p,
    ...
    def run(self, lgf_pb, inputs, hw_spec, sw_config, sim_params, graph_coll):
        ...
        inf_out_ptr = self.lib.Run(self._obj, spec_data, len(spec_data), sw_config_data,
                                   len(sw_config_data), params_data, len(params_data),
                                   lgf_data, len(lgf_data), input_data, len(input_data),
                                   graph_coll.pointer(), ctypes.byref(inf_out_size))
...
def run_inference(lgf_pb, inputs, hw_spec, sw_config, sim_params, graph_coll=None):
    ...
    with PyInferenceRunner() as py_inf_runner:
        inf_out = py_inf_runner.run(lgf_pb, inputs, hw_spec, sw_config, sim_params,
                                    graph_coll)
