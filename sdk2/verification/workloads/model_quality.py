...
from sdk2.verification.tools import compute_mAP, object_detection_utils
...
class BoundingBox(object):
    ...
    def format_to_string(self):
        return "{0} {1} {2} {3} {4} {5}\n".format(self._det_cls, self._score,
    ...
    def update_quality_metrics(self, performance_data, test_outputs, labels):
        ...
        for batch in test_outputs.batches:
            for j in range(batch.results[0].edge_info.shape.d[0]):
                ...
                shutil.copy2(os.path.join(test_labels_dir, label_name),
                             self._ground_truth_dir)
                img_ind += 1

        all_results = []
        for inf_out in test_outputs.batches:
            these = self.inference_to_obj_det_results(inf_out, test_images_sizes,
                                                      self._image_indx)
