...
from sdk2.proto import graph_types_pb2, node_filters
from sdk2.verification.tools import object_detection_utils
from sdk2.verification.workloads import model_quality, standard_data_base
...
class COCOSweep(standard_data_base.StandardDataBase,
    ...
    def get_test_labels(self, shard):
        ...
        test_images_sizes = np.load(
            os.path.join(self._data_dir, self.TEST_IMAGES_SIZES_FILE_NAME))
    ...
    def inference_to_obj_det_results(self, inf_out, test_images_sizes, start_img):
        ...
        for j in range(arrays[BOXES].shape[0]):
            ...
            for i in range(int(arrays[NUM][j])):
                ...
                new_box = np.array([
                    box[0] * orig_size[0], box[1] * orig_size[1], box[2] * orig_size[0],
                    box[3] * orig_size[1]
                ])
