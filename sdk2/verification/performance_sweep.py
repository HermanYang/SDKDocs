...
from sdk2.graph.run_graph import graph_runner, histogram_graph_runner
from sdk2.graph.transform_graph.graph_transformers import convert_to_debug_mode
from sdk2.proto import graph_types_pb2, performance_data_pb2, sim_params_pb2
from sdk2.visuals import sim_result_to_trace
...
class PerformanceSweep(object):
    ...
    def get_importer(self, sw_config):
        input_edges = (full_graph_pipeline.extract_edge_from_data(
            self.get_test_inputs(sw_config, 0)))
        return graph_importer_map.GRAPH_IMPORTER_MAP[self._graph_type](
            self._graph_path, sw_config, input_edges=input_edges)
    ...
    def _save_debug_info(self,
        ...
        if sw_config.debug_info.debug_dir:
            ...
            lgf_graph.LightGraph.write_lgf_pb(
                ...
                os.path.join(sw_config.debug_info.debug_dir, "lgf.pb"))

            if sw_config.sweep_info.collect_memory_layout:
                with open(os.path.join(sw_config.debug_info.debug_dir, "mem_layout.pb"),
                          "wb") as f:
            ...
            if sw_config.sweep_info.convert_graph_to_debug_mode:
                ...
                for key, cal_hist_pb in cal_hist_pb_map.items():
                    hist_path = os.path.join(protobuf_dir,
                                             plot_title_map.get(key, str(key)) + ".pb")
    ...
    def _run_streamed_test_data(self, runner, performance_data):
        ...
        for shard_indx in range(self.num_test_shards()):
            ...
            self.update_quality_metrics(performance_data, test_outputs,
    ...
    def _run_single_config_helper(self, performance_data):
        ...
        if performance_data.config.do_transform:
            ...
            full_graph_pipeline.main(self._graph_path, self._graph_type, lgf_pb_path,
                                     graph_types_pb2.LGFProtobuf, calibration_data,
                                     transform_hw_specs, transform_sw_config,
        ...
        if (performance_data.config.do_fine_tuning
            ...
            for i, shard_indx in enumerate(shard_list):
                ...
                fine_tuning_labels = self.get_fine_tuning_labels(
                    performance_data.config.sw_config, shard_indx)
                light_graph = self._fine_tuning_fn(light_graph, fine_tuning_data,
                                                   fine_tuning_labels,
        ...
        if os.path.exists(performance_data_path):
            logging.warning(
                ("Found performance data on disk, skipping configuration {0}. " +
                 ...
                 "following file: {1}").format(indx, performance_data_path))
