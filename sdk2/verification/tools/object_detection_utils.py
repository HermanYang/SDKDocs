...
def non_max_suppression(predictions_with_boxes, confidence_threshold, iou_threshold=0.4):
    ...
    :param predictions_with_boxes: 3D numpy array, first 4 values in 3rd dimension are
     bbox attrs, 5th is confidence
    :param confidence_threshold: the threshold for deciding if prediction is valid
    :param iou_threshold: the threshold for deciding if two boxes overlap
    :return: dict: class -> [(box, score)]
    """
    conf_mask = np.expand_dims((predictions_with_boxes[:, :, 4] > confidence_threshold),
                               -1)
    ...
    for i, image_pred in enumerate(predictions):
        ...
        non_zero_idxs, _ = np.nonzero(image_pred)
        image_pred = image_pred[list(set(non_zero_idxs)), :]
...
def letter_box_image(image: Image.Image, output_height: int, output_width: int,
    ...
    :return: numpy image fit within letterbox. dtype=uint8,
    ...
    fit_image = np.asarray(image.resize((fit_width, fit_height),
...
def letter_box_pos_to_original_pos(letter_pos, current_size,
    ...
    """
    Parameters should have same shape and dimension space.
    (Width, Height) or (Height, Width)
    :param letter_pos: The current position within letterbox image including fill
                       value area.
